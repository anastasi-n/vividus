...
public class ResourceValidator
{
    ...
    private Set<Integer> allowedStatusCodes = Set.of(HttpStatus.SC_OK);
    private Set<Integer> notAllowedHeadStatusCodes = Set.of(HttpStatus.SC_METHOD_NOT_ALLOWED,
        ...
        return cache.compute(resourceValidation.getUri(), (u, rv) -> {
            return Optional.ofNullable(rv).map(r -> {
                ...
                try
                {
                    ...
                    Matcher<Object> oneOf = isOneOf(allowedStatusCodes.toArray());
    ...
    private HttpResponse executeHttpMethod(HttpMethod httpMethod, HttpClientContext httpClientContext, URI uri)
            throws IOException
    {
        HttpRequestBase request = httpMethod.createEmptyRequest();
        request.setURI(uri);
